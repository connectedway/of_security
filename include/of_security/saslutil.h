 ...
 *  SASL_OK on success
 */
LIBSASL_API OFC_ULONG of_security_get_random(OFC_VOID) ;


LIBSASL_API int of_security_decode64(const char *in, unsigned inlen,
 ...
 */
LIBSASL_API int of_security_encode64(const char *in, unsigned inlen,
				   char *out, unsigned outmax, 
				   unsigned *outlen);
...
LIBSASL_API void of_security_randfree(sasl_rand_t **rpool);

/* seed random number generator */
LIBSASL_API void of_security_randseed(sasl_rand_t *rpool, const char *seed,
				    unsigned len);

/* generate random octets */
LIBSASL_API void of_security_rand(sasl_rand_t *rpool, char *buf, unsigned len);

/* churn data into random number generator */
LIBSASL_API void of_security_churn(sasl_rand_t *rpool, const char *data,
				 unsigned len);

/* erase a security sensitive buffer or password.
 *   Implementation may use recovery-resistant erase logic.  
 */
LIBSASL_API void of_security_erasebuffer(char *pass, unsigned len);

/* Lowercase string in place */
LIBSASL_API char *of_security_strlower (char *val);

LIBSASL_API int of_security_config_init(const char *filename);
