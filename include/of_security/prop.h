 ...
 */
LIBSASL_API struct propctx *of_security_prop_new(unsigned estimate);
 ...
 */
LIBSASL_API int of_security_prop_dup(struct propctx *src_ctx, struct propctx **dst_ctx);
 ...
 * possible other return values include: SASL_NOMEM, SASL_BADPARAM
 */
LIBSASL_API int of_security_prop_request(struct propctx *ctx, const char **names);
 ...
 */
LIBSASL_API const struct propval *of_security_prop_get(struct propctx *ctx);
 ...
 */
LIBSASL_API int of_security_prop_getnames(struct propctx *ctx, const char **names,
				   struct propval *vals);
 ...
 */
LIBSASL_API void of_security_prop_clear(struct propctx *ctx, int requests);

/* erase the value of a property
 */
LIBSASL_API void of_security_prop_erase(struct propctx *ctx, const char *name);

/* dispose of property context
 *  ctx      -- is disposed and set to NULL; noop if ctx or *ctx is NULL
 */
LIBSASL_API void of_security_prop_dispose(struct propctx **ctx);
 ...
 */
LIBSASL_API int of_security_prop_format(struct propctx *ctx, const char *sep, 
				 int seplen,
 ...
 */
LIBSASL_API int of_security_prop_set(struct propctx *ctx, const char *name,
 ...
 * possible error return values include: SASL_BADPARAM, SASL_NOMEM
 */
LIBSASL_API int of_security_prop_setvals(struct propctx *ctx, const char *name,
				  const char **values);
