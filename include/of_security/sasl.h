/**
 * \defgroup of_security Open Files Authentication Library
 *
 * The Open Files Authentication library provides SASL support for realtime 
 * applications.  It currently supports NTLMSSP with and without GSSAPI/SPNEGo
 * wrappers.
 *
 * \{
 */
...
LIBSASL_API void of_security_set_alloc(sasl_malloc_t *,
				     sasl_calloc_t *,
				     sasl_realloc_t *,
				     sasl_free_t *);
...
LIBSASL_API void of_security_set_mutex(sasl_mutex_alloc_t *, 
				     sasl_mutex_lock_t *,
				     sasl_mutex_unlock_t *, 
				     sasl_mutex_free_t *);
...
#define SASL_CB_TIMESTAMP    0x4009  /* Timestamp for negotiation */
 ...
 */  
LIBSASL_API int of_security_set_path (int path_type, char * path);
...
LIBSASL_API void of_security_version(const char **implementation,
				   int *version);
 ...
 */
LIBSASL_API void of_security_version_info (const char **implementation,
 ...
 * sasl_client_done.
 */
LIBSASL_API void of_security_done(void);
 ...
 * This function should be called instead of sasl_done(),
   whenever possible.
 */
LIBSASL_API int of_security_server_done(void);
 ...
 * This function should be called instead of sasl_done(),
   whenever possible.
 */
LIBSASL_API int of_security_client_done(void);

/* dispose connection state, sets it to NULL
 *  checks for pointer to NULL
 */
LIBSASL_API void of_security_dispose(sasl_conn_t **pconn);
 ...
 */
LIBSASL_API const char *of_security_errstring(int saslerr,
					    const char *langlist,
					    const char **outlang);
 ...
 */
LIBSASL_API const char *of_security_errdetail(sasl_conn_t *conn);
 ...
 */
LIBSASL_API void of_security_seterror(sasl_conn_t *conn, unsigned flags,
			       const char *fmt, ...);
 ...
 */
LIBSASL_API int of_security_getprop(sasl_conn_t *conn, int propnum,
			     const void **pvalue);
 ...
 */
LIBSASL_API int of_security_setprop(sasl_conn_t *conn,
				  int propnum,
				  const void *value);
 ...
 */
LIBSASL_API int of_security_idle(sasl_conn_t *conn);
...
typedef struct sasl_interact {
    ...
    void *result;		/* set to point to result */
 ...
 *  ...
 */
LIBSASL_API int of_security_client_init(const sasl_callback_t *callbacks);
 ...
 *  SASL_NOMEM    -- not enough memory
 */
LIBSASL_API int of_security_client_new(const char *service,
 ...
 */
LIBSASL_API int of_security_client_start(sasl_conn_t *conn,
				       const char *mechlist,
 ...
 */
LIBSASL_API int of_security_client_step(sasl_conn_t *conn,
				      const char *serverin,
 ...
 *  SASL_BADVERS   -- Mechanism version mismatch
 */
LIBSASL_API int of_security_server_init(const sasl_callback_t *callbacks,
				      const char *appname);
 ...
 *  SASL_NOMEM     -- not enough memory
 */
LIBSASL_API int of_security_server_new(const char *service,
				const char *serverFQDN,
				const char *user_realm,
				const char *iplocalport,
				const char *ipremoteport,
...
LIBSASL_API const char ** of_security_global_listmech(void);
 ...
 */
LIBSASL_API int of_security_listmech(sasl_conn_t *conn,
				   const char *user,
				   const char *prefix,
				   const char *sep,
				   const char *suffix,
 ...
 */
LIBSASL_API int of_security_server_start(sasl_conn_t *conn,
				  const char *mech,
				  const char *clientin,
 ...
 *  ...
 */
LIBSASL_API int of_security_server_step(sasl_conn_t *conn,
				 const char *clientin,
				 unsigned clientinlen,
				 const char **serverout,
				 unsigned *serveroutlen);
 ...
 */
LIBSASL_API int of_security_checkapop(sasl_conn_t *conn,
 ...
 *  SASL_NOUSER   -- user not found
 */
LIBSASL_API int of_security_checkpass(sasl_conn_t *conn,
 ...
 */
LIBSASL_API int of_security_user_exists(sasl_conn_t *conn,
				 const char *service,
				 const char *user_realm,
				 const char *user);
 ...
 *  SASL_OK        -- successful
 */
LIBSASL_API int of_security_setpass(sasl_conn_t *conn,
				  const char *user,
				  const char *pass, unsigned passlen,
...
#define SASL_AUX_END      OFC_NULL	/* last auxiliary property */
 ...
 */
LIBSASL_API int of_security_auxprop_request(sasl_conn_t *conn,
 ...
 */
LIBSASL_API struct propctx *of_security_auxprop_getctx(sasl_conn_t *conn);
 ...
 */
LIBSASL_API int of_security_auxprop_store(sasl_conn_t *conn,
 ...
 */
LIBSASL_API int of_security_encode(sasl_conn_t *conn,
 ...
 */
LIBSASL_API int of_security_encodev(sasl_conn_t *conn,
				  const OFC_IOVEC *invec, unsigned numiov,
				  const char **output, unsigned *outputlen);
 ...
 */
LIBSASL_API int of_security_decode(sasl_conn_t *conn,
				 const char *input, unsigned inputlen,
				 const char **output, unsigned *outputlen);
